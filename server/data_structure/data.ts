...
const roadColorMinRed = 67
const roadColorMinGreen = 255
const roadColorMinBlue = 40
const roadColorMaxRed = 208
const roadColorMaxGreen = 2
const roadColorMaxBlue = 27
...
export class Series {
    ...
    public effect: any
    public tooltip: any
    constructor(roadLine: [], speed?: number, color?: string, max?: number) {
        this.data = [{
            coords: roadLine,
        }]
        this.type = 'lines'
...
export function getColor(value: number, max: number) {
    ...
    let color = 'rgb(0, 0, 0)'
    // 计算颜色
    let r = Math.floor(roadColorMinRed + (roadColorMaxRed - roadColorMinRed) * value / max)
    let g = Math.floor(roadColorMinGreen + (roadColorMaxGreen - roadColorMinGreen) * value / max)
    let b = Math.floor(roadColorMinBlue + (roadColorMaxBlue - roadColorMinBlue) * value / max)
    color = `rgb(${r}, ${g}, ${b})`
    // console.log(color);
    return color
}
